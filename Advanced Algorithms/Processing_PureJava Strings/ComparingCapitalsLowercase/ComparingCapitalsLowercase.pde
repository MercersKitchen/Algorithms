...
void setup() {
  ...
  println("Beginning Sentence,", Misc.sentence, ", with", Misc.sentence.length(), "characters");
  println("Beginning Phrase,", Misc.phrase, ", with", Misc.phrase.length(), "characters", "\n");
  ...
  println(Misc.phrase);
  for (int i=0; i<Misc.phrase.length(); i++) {
    if (i>9) {
      print(i-10);
    } else {
      print(i);
